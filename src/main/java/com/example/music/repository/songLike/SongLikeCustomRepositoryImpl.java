...
public class SongLikeCustomRepositoryImpl implements SongLikeCustomRepository {
    ...
    public Flux<SongLikeDto> findTopLikeCount(Integer top) {
        String sql = String.format(
                """
                SELECT s.song_id as song_id, s.song_name as song_name, COUNT(sl.song_like_id) as likes
                FROM song_likes sl
                JOIN songs s
        ...
        return client.sql(sql)
                .map((row, metadata) -> new SongLikeDto(
                        (Long) row.get("song_id"),
                        (String) row.get("song_name"),
                        (Long) row.get("likes"))
                )
                .all();
    }
